...
def getIBMValidatedOpenSourceList(oskey):
	...
	try:
		...
		if oskey == 'Ubuntu_20' or oskey == 'all':
			oskey_match = True
			ubuntu20_list = getIBMValidatedSoftwareList(data=data_json,oskey='Ubuntu 20.x')
			q = 'IBM_Validated_OSS_List_Ubuntu_2004.json'
			file_name = f'{DATA_FILE_LOCATION}/{q}'
			with open(file_name,'w') as file:
				json.dump(ubuntu20_list,file,indent=2)
				print(f"Saved!\nfilename: {q}")
				
		if oskey == 'Ubuntu_22' or oskey == 'all':
			oskey_match = True
			ubuntu22_list = getIBMValidatedSoftwareList(data=data_json,oskey='Ubuntu 22.x')
			q = 'IBM_Validated_OSS_List_Ubuntu_2204.json'
			file_name = f'{DATA_FILE_LOCATION}/{q}'
			with open(file_name,'w') as file:
				json.dump(ubuntu22_list,file,indent=2)
				print(f"Saved!\nfilename: {q}")
